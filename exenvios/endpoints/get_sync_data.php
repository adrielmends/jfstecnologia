...
try {
    ...
    // Lazy migration: Ensure capacity column exists
    try { $db->exec("ALTER TABLE `lockers` ADD COLUMN `capacity` INT DEFAULT 10 AFTER `status` "); } catch(Exception $e) {}
    ...
    // Fetch Lockers with occupancy and capacity
    $sqlLockers = "SELECT l.*, loc.name as location_name,
                   (SELECT COUNT(*) FROM orders o WHERE (o.locker_id = l.id OR o.locker_id = l.label) AND o.status NOT IN ('delivered', 'failed')) as occupied_count 
                   FROM `lockers` l 
